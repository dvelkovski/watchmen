[#include "/watchmen/templates/macros/fieldsMacro.ftl"/]
...
<div id="events-wrapper" class="events-wrapper">
    ...
    [#if events?has_content]
        [#list cmsfn.asContentMapList(events?children) as event]
            <span class="event" data-start="[#if event.startDate?has_content]${event.startDate?string["yyyy-MM-dd"]}"[/#if]
                  data-nodename="${event.@name}" data-end="[#if event.endDate?has_content]${event.endDate?string["yyyy-MM-dd"]}"[/#if] data-color="${event.color!}" data-title="${event.title!}" data-category="${event.category!}"></span>
        [/#list]
    [/#if]
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-inner">
                ...
                [@textField name="eventEndDate" id="existingEventEndDate" label="Event start date" class="datePicker" hint="Enter event end date" type="text"/]
            ...
            <div class="modal-footer">
                <p class="text-right"><button class="btn btn-flat btn-brand waves-attach waves-button waves-effect" data-dismiss="modal" type="button">Close</button><button class="btn btn-flat btn-brand waves-attach waves-button waves-effect" data-dismiss="modal" id="updateEventFromCal" type="button">OK</button></p>
            </div>
        </div>
    </div>
</div>
